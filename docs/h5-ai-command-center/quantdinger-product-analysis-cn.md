...
10 分价值升级后，产品定位要从 `AI Trading Command Center` 进一步升级为 `AI Trading Flight Control System`：不是让 AI 更自由地下单，而是让系统像飞控一样限制、校验、报警、记录、纠偏。它的第一性指标不是信号数量，而是坏交易减少、最大亏损可知、风控拒绝质量、场景准备完整度和复盘转化率。

## 10 分价值标准

| 维度 | 8 分系统 | 10 分系统 |
|---|---|---|
| Research | 能总结信息 | 能输出有来源、时间戳、反方观点、交易影响和过期提示的 evidence packet |
| H5 判断 | 能评分和给信号 | 能解释为什么等、为什么不做、哪里失效、最多亏多少 |
| 风控 | 能设置止损 | 能账户级降级、冻结、利润锁定、策略预算耗尽后停止 |
| 执行 | 能生成订单意图 | 能处理过期、部分成交、撤单失败、异常冻结和审计追踪 |
| 复盘 | 能写交易日志 | 能把每次信号、拒绝、等待、止损、错过机会转成候选改进 |
| 护城河 | 像一个交易平台 | 沉淀 Stark 的 H5 思想、真实决策数据、失败画像和私有情境库 |

P0 因此要更窄：Research Evidence、H5 StrategySpec、Scenario Planner、Risk Kernel、Signal/Paper、Decision Ledger、Weekly Review。其它功能可以重要，但不能抢在这条价值闭环前面。
